...
public class Tabla implements ActionListener  {
    ...
    public ImageIcon piesaRosu = new ImageIcon("red.png");
    public ImageIcon piesaNegru = new ImageIcon("black.png");
    public ImageIcon regeRosu = new ImageIcon("regeRosu.png");
    public ImageIcon regeNegru = new ImageIcon("regeNegru.png");
...
public Tabla(){
           ...
           if ((i+j)%2==0){
               ...
               p.add(a[i][j]);
                 ...
                 if  ((i<=2) || (i>=7)){
                    ...
                    A[i][j].culoare=0;
                    A[i][j].rege=false;
                    a[i][j].setBackground(Color.lightGray);
                    p.add(a[i][j]);
                     }
                     else{
    ...
    nume = new JLabel(ex.ob.nume.getText(),JLabel.CENTER);
    nume.setFont(new Font("Arial",Font.BOLD,14));
    computer = new JLabel("Computer",JLabel.CENTER);
    computer.setFont(new Font("Arial",Font.BOLD,14));
    punctaj = new JLabel("Scor",JLabel.CENTER);
    punctaj.setFont(new Font("Arial",Font.BOLD,14));
    piese = new JLabel("Numar piese",JLabel.CENTER);
    piese.setFont(new Font("Arial",Font.BOLD,14));
    regi = new JLabel("Numar regi",JLabel.CENTER);
    regi.setFont(new Font("Arial",Font.BOLD,14));
    punctajn = new JLabel("0",JLabel.CENTER);
    punctajn.setFont(new Font("Arial",Font.BOLD,14));
    punctajc = new JLabel("0",JLabel.CENTER);
    punctajc.setFont(new Font("Arial",Font.BOLD,14));
    piesen = new JLabel("8",JLabel.CENTER);
    piesen.setFont(new Font("Arial",Font.BOLD,14));
    piesec = new JLabel("8",JLabel.CENTER);
    piesec.setFont(new Font("Arial",Font.BOLD,14));
    regin = new JLabel("0",JLabel.CENTER);
    regin.setFont(new Font("Arial",Font.BOLD,14));
    regic = new JLabel("0",JLabel.CENTER);
    regic.setFont(new Font("Arial",Font.BOLD,14));
...
public int minimax( Patrat[][] P, int ad){
   ...
   if (ad>0){
        if (max==true){//computerul este la mutare 
            max=false;
        if (ex.ob.rosu.getState()){  //computerul joaca cu piesele negre
            toate_mutarile_n(P); 
        ...
        if (ex.ob.negru.getState()){  //computerul joaca cu piesele albe
            toate_mutarile_a(P); 
            for (ccc=1;ccc<=cnt;ccc++){
                r++;
                m1[r]=new Mutare();
           ...
           max=true;   
           if (ex.ob.rosu.getState()){  //nume joaca cu piesele albe
            toate_mutarile_a(P); 
            for (ccc=1;ccc<=cnt;ccc++){
                r++;
                m1[r]=new Mutare();
        ...
        if (ex.ob.negru.getState()){  //nume joaca cu piesele negre
            toate_mutarile_n(P); 
            for (ccc=1;ccc<=cnt;ccc++){
                r++;
                m1[r]=new Mutare();
   ...
   if (ad==0){      
        if (ex.ob.rosu.getState()){  //com joaca cu piesele negre
           id=1;
           m1[id]=new Mutare();
           m1[id].eval=evaluare1(P,1);   
        }
        if (ex.ob.negru.getState()){  //com joaca cu piesele negre
           id=1;
           m1[id]=new Mutare();
...
public void muta_computer(){  
  ...
  Mutare xx;
    nmrn=nr_piese_negre();
    nmra=nr_piese_albe();
    if (ex.ob.usor.getState()){
    if (ex.ob.rosu.getState()){
      if (nmrn==0){
            JOptionPane.showMessageDialog(fereastraPrincipala,ex.ob.nume.getText()+" a castigat");
            for (ci=1;ci<=8;ci++)
              for (cj=1;cj<=8;cj++){ 
                a[ci][cj].setEnabled(false);
              }  
